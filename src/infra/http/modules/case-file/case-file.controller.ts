...
export class CaseFileController {
  constructor(
    ...
    private readonly deleteCaseFileUseCase: DeleteCaseFileUseCase,
  ) {}
  ...
  @Delete(':id')
  async deleteCaseFile(@Param('id') caseFileId: string) {
    await this.deleteCaseFileUseCase.execute({ caseFileId });
  }
}
